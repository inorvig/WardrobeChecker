 ...
 * in document order.
 * <p>
 * Members can be added using one of the different <code>add(...)</code>
 * methods. Accepted values are either instances of {@link JsonValue}, or
 * strings, primitive numbers, or boolean values.
 * </p>
 * <p>
 * Members can be accessed by their name using {@link #get(String)}. A list of
 * all names can be obtained from the method {@link #names()}.
 * </p>
 * <p>
...
public class JsonObject extends JsonValue {
	...
	/**
	 * Adds a new member to this object, with the specified name and the JSON
	 * representation of the specified <code>long</code> value.
	 * <p>
	 * This method <strong>does not prevent duplicate names</strong>. Adding a
	 * member with a name that is already contained in the object will add
	 * another member with the same name. In order to ensure that the names are
	 * unique, the method <code>remove( String )</code> can be called before
	 * calling this method. However, this practice incurs a performance penalty
	 * and should only be used when the calling code can not ensure that same
	 * name won't be added more than once.
	 * </p>
	 * 
	 * @param name
	 ...
	 * contains multiple members with the given name, only the first one is
	 ...
	 * @return the value of the member with the specified name, or
	 *         <code>null</code> if there is no member with that name in this
	 *         object
	 */
