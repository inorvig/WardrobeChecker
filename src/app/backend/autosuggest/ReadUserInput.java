...
public class ReadUserInput {
    ...
    ArrayList<String> finalFive = new ArrayList<String>(); // the final five results to be returned sit in this list (space-saving?)
     ...
     * @param unigram
     * @param bigram
     * @param smartOn : boolean indicating whether smart ranking has been asked for by user
     */
    public ReadUserInput(Prefix prefix, LED led, Whitespace whitespace, Trie trie, HashMap<String, Integer> unigram, HashMap<String, Integer> bigram) {
        this.prefix = prefix;
        this.led = led;
    ...
    /**
     * method that returns the list of suggestion to be displayed by finding the best suggestions 
     * using relevant features
     * @param userTyped : input string typed in by user
    ...
    public ArrayList<String> getList(String userTyped){
		...
		while (tokenizer.hasMoreTokens())
	        ...
	        String toPrepend = ""; //String that is needed to correctly print out results for repl or gui 
	        if (listSize>1){
	        ...
	        TreeSet<String> results; // tree set structure passed to all features to insert results using comparator
	        
	        // use regular comparator is smart off
	        	RegComparator regCompare = new RegComparator(lastWord, penultimateWord, unigramMap, bigramMap);//lastWord, secondLastWord, wrfwrfe);
	        ...
	        if (!prefix.prefixOn && !led.ledOn && !whitespace.whitespaceOn){
		        ...
		        else finalFive.add(toPrepend);
	    		
		        return finalFive;
		    }
	        

	        //pass tree set to the feature classes to add relevant results to the set
	        ...
	        HashSet<String> tempSet = new HashSet<String>(); // temp set to bypass bug in the regComparator
	        String toAdd, fullAdd; //temporary helper strings to avoid polling more than is necessary
	        int counter = 0;
	        while (counter < 5){
	        	if (!results.isEmpty()){
