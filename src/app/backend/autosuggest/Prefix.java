...
public class Prefix implements Feature, Serializable {
    ...
    /**
     * constructor method
     * @param trie: trie to be used for prefix matching is passed in from main class for better design
     */
    ...
    public void resultSet(Set<String> results, String word){ // returns the inputted hashset of all results found with given prefix in trie
	    if (prefixOn){
	        ...
	        boolean inTrie = true; // boolean that shows if current collection of letters is in the trie
	        for (int i=0; i<wordLength; i++){ //iterate through letters in word
	        ...
	        if (inTrie){
	            getResults(currNode,results,word); // helper method that builds set of suggestions
	        }
        }else{
        	//do nothing
        }
    ...
    public void getResults(Node node, Set<String> results, String word){
        ...
        for (Character letter : children.keySet()){ // iterate using keys of hashmap of children
            currNode = node.getChild(letter);
            newWord = word + letter.toString(); // append letter to end of word and pass into method as new word
            getResults(currNode, results, newWord); // recursively build all word combinations with given prefix in the trie
        } 
   }
