...
public class LED implements Feature {
	...
	public boolean ledOn; // boolean to mark if LED search has been turned on by
							// user
	 ...
	 * 
	 * @param unigramMap
	 *            : save space by reusing the unigram map built in main class
	 */
	...
	/**
	 * @param distance
	 *            : integer passed in is set as the levenshtein edit distance.
	 *            Int is checked before being passed.
	 */
	...
	public void resultSet(TreeSet<String> results, String word) {
		if (ledOn) {
			for (String entry : wordMap.keySet()) { // iterating through keyset
													// of all words in corpus
				if (distance(word, entry) <= led) // finding all words in corpus
													// that are less than or
													// equal to led away from
													// input word
					results.add(entry);
			}
		} else {
	...
	public int distance(String word, String entry) {
		...
		for (int i = 1; i <= wordLength; i++) {
			...
			for (int j = 1; j <= entryLength; j++) {
				...
				if (ithLetterInWord == jthLetterInEntry)
					cost = 0;
				else
					cost = 1;
